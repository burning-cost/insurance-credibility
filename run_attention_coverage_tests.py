"""
Run test_attention_coverage.py on Databricks serverless compute.
"""
...
WORKSPACE_DIR = "/Workspace/Shared/insurance-credibility-attention-cov"
...
# MAGIC %pip install polars numpy pytest torch
...
LOCAL_DIR = tempfile.mkdtemp(prefix="ins_cred_attn_")
...
result = subprocess.run(
    [sys.executable, "-m", "pytest",
     LOCAL_DIR + "/tests/test_attention_coverage.py",
...
NOTEBOOK_PATH = f"{WORKSPACE_DIR}/_run_attention_coverage"
...
run_response = w.jobs.submit(
    run_name="insurance-credibility-attention-coverage",
    tasks=[
        jobs.SubmitTask(
            task_key="pytest",
    ...
    environments=[
        jobs.JobEnvironment(
            ...
            spec=jobs.compute.Environment(
                client="2",
                dependencies=["polars", "numpy", "pytest", "torch"],
            ),
        )
    ],
)
