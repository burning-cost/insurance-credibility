"""
Run pytest for insurance-credibility on Databricks serverless compute.
...
# Use tempfile to find writable location; disable pytest cache to avoid __pycache__ writes

NOTEBOOK_CONTENT = f"""\
# Databricks notebook source
# MAGIC %pip install scipy polars numpy pandas pyarrow pytest
...
LOCAL_DIR = tempfile.mkdtemp(prefix="ins_cred_")
...
# Run pytest with no-cacheprovider to avoid writing __pycache__
...
run_response = w.jobs.submit(
    run_name="insurance-credibility-pytest",
    tasks=[
        jobs.SubmitTask(
            task_key="pytest",
