"""
Run ONLY tests/test_classical_validation.py on Databricks serverless compute.
"""
...
WORKSPACE_DIR = "/Workspace/Shared/insurance-credibility-test"
...
subprocess.run(
    ["pip", "install", "--quiet", "scipy", "polars", "numpy", "pandas", "pyarrow", "pytest"],
    check=True, capture_output=True
)
print("Dependencies installed")

WORKSPACE_SRC = "{WORKSPACE_DIR}"
LOCAL_DIR = tempfile.mkdtemp(prefix="ins_cred_val_")
...
# Run only the validation tests first
result = subprocess.run(
    [sys.executable, "-m", "pytest",
     LOCAL_DIR + "/tests/test_classical_validation.py",
...
if result.returncode == 0:
    print("\\n=== Validation tests PASSED. Running full suite... ===")
...
NOTEBOOK_PATH = f"{WORKSPACE_DIR}/_run_validation_tests"
...
run_response = w.jobs.submit(
    run_name="insurance-credibility-validation-pytest",
    tasks=[
        SubmitTask(
            task_key="pytest",
